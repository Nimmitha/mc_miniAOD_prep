...
execute_step() {
    ...
    if [ "$step" == "HLT" ]; then
        local cmssw_version="CMSSW_8_0_33_UL" # HLT step requires this version for 2016
    else
    ...
    directories=($(for i in ${directories[@]}; do if [[ $i =~ [12345]$ ]]; then echo $i; fi; done)) # Ex: [589]
    ...
    for directory in "${directories[@]}"; do
        if [ -d "$directory" ]; then
            ...
            if [ "$step" == "SIM" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun SIM_2016_cfg.py >> output2.log 2>&1 &
            elif [ "$step" == "DIGI" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun DIGIPremix_2016_cfg.py >> output3.log 2>&1 &
            elif [ "$step" == "HLT" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun HLT_2016_cfg.py >> output4.log 2>&1 &
            elif [ "$step" == "RECO" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun RECO_2016_cfg.py >> output5.log 2>&1 &
            elif [ "$step" == "MiniAOD" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun MINIAOD_2016_cfg.py >> output6.log 2>&1 &
            else
