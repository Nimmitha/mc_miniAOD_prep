...
execute_step() {
    ...
    if [ "$step" == "HLT" ]; then
        local cmssw_version="CMSSW_9_4_14_UL_patch1" # HLT step requires this version for 2017
    else
    ...
    for directory in "${directories[@]}"; do
        if [ -d "$directory" ]; then
            ...
            if [ "$step" == "SIM" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun SIM_2017_cfg.py >> output2.log 2>&1 &
            elif [ "$step" == "DIGI" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun DIGIPremix_2017_cfg.py >> output3.log 2>&1 &
            elif [ "$step" == "HLT" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun HLT_2017_cfg.py >> output4.log 2>&1 &
            elif [ "$step" == "RECO" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun RECO_2017_cfg.py >> output5.log 2>&1 &
            elif [ "$step" == "MiniAOD" ]; then
                echo "Working in $(pwd)"
                nohup cmsRun MINIAOD_2017_cfg.py >> output6.log 2>&1 &
            else
